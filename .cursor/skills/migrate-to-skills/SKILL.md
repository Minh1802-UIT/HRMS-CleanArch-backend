...
**Rules**: Migrate if rule has a description but NO globs and NO lwaysApply: true.
...
Changes: Add 
ame field, remove globs/lwaysApply, keep body exactly.
...
ame (from filename) and description (infer from content), keep body exactly.

## Notes

- 
ame must be lowercase with hyphens only
...
5. Write SKILL.md with new frontmatter (
ame and description) + the EXACT original body content (preserve all whitespace, formatting, code blocks verbatim)
...
4. Write SKILL.md with new frontmatter (
ame and description) + blank line + the EXACT original file content (preserve all whitespace, formatting, code blocks verbatim)
...
2. Dispatch three fast subagents in parallel to do the following steps for project rules (pattern: {workspaceFolder}/**/.cursor/rules/*.mdc), user commands (pattern: ~/.cursor/commands/*.md), and project commands (pattern: {workspaceFolder}/**/.cursor/commands/*.md):
   I. [ ] Find files to migrate in the given pattern
   II. [ ] For rules, check if it's an "applied intelligently" rule (has description, no globs, no lwaysApply: true). Commands are always migrated.
   III. [ ] Make a list of files to migrate. If empty, done.
   IV. [ ] For each file, read it, then write the new skill file preserving the body content EXACTLY.
   V. [ ] Delete the original file.
...
3. [ ] For rules, check if it's an "applied intelligently" rule (has description, no globs, no lwaysApply: true). Commands are always migrated.
